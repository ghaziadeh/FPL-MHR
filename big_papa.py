...
# #######################################################################################
# #######################################################################################
# DESIGNED BY ABDALLAH ATALLAH: send questions to Abdullahatallah2@gmail.com
...
def data_fetch(desire='tip', encoder_front=0, encoder_back=0):
    ...
    if desire == 'tip':
        ...
        if once ==0:
            time_diff = 0
        once += 1
        min_diff_between_server_and_client = 40# in milliseconds
        if time_diff < min_diff_between_server_and_client:
            today = str(date.today())
            # removing risk of client crashing due to unauthorized access to database.
            while True:
                try:
...
def manual_message_decode(action):
    f2 = open("speed.txt", "r")
    data2 = f2.readline()
    f2.close()
    try:
...
# columns : 0, 1 ,2 only
# default list example since no ROS form was given through IOS app
list_of_bins = [[1, 5, 9999, '6C:E8:5C:5C:D5:87']]
x = 0
while True:
    message = fetch_message()
    while message != "automatic mode":
        message = manual_message_decode(message)
    while message != "manual mode" and message != "stop":
        ...
        while x < len(list_of_bins):
            message = start_trip(list_of_bins[x])
            if message == 'stop':
                break
